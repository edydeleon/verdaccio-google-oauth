...
describe("Shared", () => {
  describe("buildStatusPage", () => {
    it("generic page matches the snapshot", () => {
      expect(buildStatusPage("custom test body", true)).toMatchInlineSnapshot(`
        "<!DOCTYPE html>
        <html lang=\\"en\\">
          <head>
            <title>google-oauth</title>
            <style>html, body { padding: 0; margin: 0; height: 100%; background-color: #e0e0e0; color: #24292F; font-family: Helvetica, sans-serif; position: relative; text-align: center; } .wrap { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); } a { color: #3f51b5; } .img { filter: drop-shadow(0 0.5rem 0.5rem #24292F80); width: 100px; }</style>
          </head>
          <body>
            <div class=\\"wrap\\">
    ...
    it("error page matches the snapshot", () => {
      ...
      expect(buildErrorPage(error, true)).toMatchInlineSnapshot(`
        "<!DOCTYPE html>
        <html lang=\\"en\\">
          <head>
            <title>google-oauth</title>
    ...
    it("access denied page matches the snapshot", () => {
      expect(buildAccessDeniedPage(true)).toMatchInlineSnapshot(`
        "<!DOCTYPE html>
        <html lang=\\"en\\">
          <head>
            <title>google-oauth</title>
